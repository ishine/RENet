...
def audioread(path, norm=False, start=0, stop=None, target_level=-25):
    ...
    if len(audio.shape) == 1:  # mono
        ...
        # Use left channel only (user requested)
        # soundfile returns shape (n_samples, n_channels)
        try:
            audio = audio[:, 0]
        except Exception:
            # fallback to averaging if indexing fails
            audio = audio.T
...
def snr_mixer(params, clean, noise, refclean, snr, target_level=-25, clipping_threshold=0.99):
    ...
    refclean = refclean/(max(abs(refclean))+EPS)
    refclean = normalize(refclean, target_level)
    ...
    if is_clipped(noisyspeech):
        ...
        refclean = refclean/noisyspeech_maxamplevel
    ...
    if is_clipped(refclean):
        refclean_maxamplevel = max(abs(refclean))/(clipping_threshold-EPS)
        refclean = refclean/refclean_maxamplevel
        clean = clean/refclean_maxamplevel
        noisyspeech = noisyspeech/refclean_maxamplevel
        noisenewlevel = noisenewlevel/refclean_maxamplevel
        noisy_rms_level = int(20*np.log10(scalarnoisy/refclean_maxamplevel*(rmsnoisy+EPS)))
        
    return clean, noisenewlevel, noisyspeech, noisy_rms_level, refclean
