...
import numpy as np
from streaming.transformer import TransformerBlock
...
class CwiseRMSNorm(nn.Module):
    ...
    def forward_stream(self, input_t, power_cache, count_cache):
            # input_t: (B, C, 1, F)
...
class StreamMPNet(nn.Module):
    ...
    def forward(
        ...
        """
        x = torch.stack((torch.cos(noisy_pha_t), torch.sin(noisy_pha_t), noisy_amp_t), dim=1)
        x = x.permute(0, 1, 3, 2)
