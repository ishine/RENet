...
class AttrDict(dict):
    def __init__(self, *args, **kwargs):
        ...
        self.__dict__ = self
from streaming.model import StreamMPNet
...
class MPNetStreamingEnhancer:
    def __init__(
        ...
        load_result = self.model.mpnet.load_state_dict(matched_state, strict=False)
        print(
    ...
    def enhance_waveform_stream(self, noisy_wav: np.ndarray) -> np.ndarray:
        ...
        norm = torch.sqrt(torch.tensor(noisy.shape[-1], device=self.device, dtype=noisy.dtype) / denom)
        noisy = noisy * norm
