...
# run python -m streaming.train_universal_streaming --config streaming/config_streaming.json for training
...
from streaming.model import MPNet, pesq_score, phase_losses
from models.mpd_and_metricd import MultiPeriodDiscriminator, \
     feature_loss, generator_loss, discriminator_loss, MetricDiscriminator, batch_pesq
...
def get_hierarchical_loaders_(noisy_root, clean_root, h, device):
    """
    Scans the noisy_root directory for subdirectories (types) and their sub-subdirectories (SNRs).
    Creates a DataLoader for each specific case (e.g., noisy/-5db).
    """
    loaders = {}
    if not os.path.exists(noisy_root):
        print(f"Warning: Test noisy directory {noisy_root} not found.")
    ...
    # 1. Iterate over Degradation Types (noisy, noisy_limit, etc.)
    for deg_type in sorted(os.listdir(noisy_root)):
        deg_path = os.path.join(noisy_root, deg_type)
        if not os.path.isdir(deg_path):
            continue

        # 2. Iterate over SNR levels (-5db, 0db, etc.)
        for snr in sorted(os.listdir(deg_path)):
            snr_path = os.path.join(deg_path, snr)
            if not os.path.isdir(snr_path):
                continue

            # 3. Find all wav files in this specific bucket
            wav_files = sorted(glob.glob(os.path.join(snr_path, '*.wav')))
            if not wav_files:
                continue

            # 4. Build pairs. 
            # The DNSDataset expects (fileid, noisy_path). 
            # We use the relative path from the root as 'fileid' so the dataset finds the matching clean file.
            # Example: fileid = "noisy/-5db/file_001.wav"
            pairs = []
            for w in wav_files:
                rel_path = os.path.relpath(w, noisy_root)
                pairs.append((rel_path, w))

            # 5. Create Dataset and Loader for this subcase
            # Note: clean_root must correspond to the root so that os.path.join(clean_root, rel_path) works
            ds = DNSDataset(pairs, clean_root, h.segment_size, h.sampling_rate,
                            split=False, shuffle=False, n_cache_reuse=0, device=device)
            ...
            # Key format: "noisy_limit_-5db"
            loaders[f"{deg_type}_{snr}"] = loader
...
def train(rank, a, h):
    ...
    if cp_g is None or cp_do is None:
        ...
        # Flexible loading for generator
        model_state_dict = generator.state_dict()
        checkpoint_state_dict = state_dict_g['generator']
...
def main():
    ...
    parser.add_argument('--input_clean_wavs_dir', default='/data2/wangchengzhong/challenge/clean_largescale')
    
    # NEW ARGUMENTS FOR HIERARCHICAL TESTING
    parser.add_argument('--test_noisy_dir', default='/data2/wangchengzhong/challenge/noisy_test')
    parser.add_argument('--test_clean_dir', default='/data2/wangchengzhong/challenge/clean_test')

    parser.add_argument('--input_training_file', default='WHAMR_largescale/training.txt')
    parser.add_argument('--input_validation_file', default='VoiceBank+DEMAND/test.txt') # This arg is kept but effectively unused for validation logic now
    parser.add_argument('--checkpoint_path', default='cp_model_streaming')
